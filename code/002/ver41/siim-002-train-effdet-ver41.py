...
sys.path.append(os.path.join(ROOT_DIR, 'input/weightedboxesfusion'))
...
class CFG:
    ...
    model_pretrained_weight = os.path.join(
        ROOT_DIR, 'input/efficientdet/efficientdet_d5-ef44aea8.pth')
    target_size = 1
...
def run_nms(predictions, image_index, image_size=None, iou_thr=0.45, skip_box_thr=None):
    boxes = [prediction[image_index]['boxes'] for prediction in predictions]
    ...
    for label in np.unique(labels):
        ...
        keep, count = nms(boxes, scores, overlap=iou_thr, top_k=100)
...
def run_wbf(predictions, image_index, image_size=CFG.size, iou_thr=0.45, skip_box_thr=0.43, weights=None):
...
def make_predictions(images, models, device, score_threshold, tta_transforms):
    predictions = []
    with torch.no_grad():
        for num_model, model in enumerate(models):
            predictions_eachmodel = []
            outputs = model(tta_transforms[num_model].batch_augment(images.clone()), image_scales=torch.tensor([1] * images.shape[0]).float().cuda())
            for i in range(images.shape[0]):
                ...
                boxes = np.clip(boxes, 0., 512.)
                boxes = boxes[indexes]
                boxes = tta_transforms[num_model].deaugment_boxes(boxes.copy())
                scores = scores[indexes]
                labels = labels[indexes]
                predictions_eachmodel.append({
                    'boxes': boxes,
                    'scores': scores
                })
            predictions.append(predictions_eachmodel)

    return predictions


def inference(valid_loader, models, device, thresh):
    predictions = []
    tta_transforms = []
    for tta_combination in product([None, TTAHorizontalFlip()],
                                   [None, TTAVerticalFlip()],
                                   [None, TTARotate90()]):
        tta_transforms.append(TTACompose([tta_transform for tta_transform in tta_combination if tta_transform]))
    tk0 = tqdm(enumerate(valid_loader), total=len(valid_loader))
    for step, (image_ids, images, targets) in tk0:
        images = torch.stack(images).float().to(device)
        predictions_temp = make_predictions(images, models, device, score_threshold=1e-7, tta_transforms=tta_transforms)
        for i in range(images.shape[0]):
            boxes, scores, labels = run_nms(predictions_temp, image_index=i, iou_thr=thresh, skip_box_thr=0.01)
...
def train_loop(folds, fold):
    ...
    for thresh in iou_thresh:
        LOGGER.info(f'iou_thresh: {thresh}')
        for tta in range(CFG.tta):
            LOGGER.info(f'tta: {tta + 1}')
            models = []
            model = get_model_inference(model_path=os.path.join(
                '/home/yuki/Kaggle-SIIM-FISABIO-RSNA/output/002/ver37', f'{CFG.model_name}_fold{fold}_best.pth'))
            ...
            test_loader = DataLoader(test_dataset,
                                     **CFG.loader_params['valid'],
                                     collate_fn=collate_fn)
            predictions = inference(test_loader, models, device, thresh=thresh)
            annotations_ = pd.DataFrame(annotations, columns=['id', 'detection_label', 'xmin', 'ymin', 'xmax', 'ymax'])
            predictions_ = pd.DataFrame(predictions, columns=['id', 'detection_label', 'conf', 'xmin', 'ymin', 'xmax', 'ymax'])
            ...
            LOGGER.info(f'class: none  mAP: {AP["0"][0]:.6f}')
            LOGGER.info(f'class: opacity  mAP: {AP["1"][0]:.6f}')
