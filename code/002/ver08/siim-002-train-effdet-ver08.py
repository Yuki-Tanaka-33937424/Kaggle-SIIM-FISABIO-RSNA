...
class CFG:
    ...
    model_pretrained_weight = os.path.join(ROOT_DIR, 'input/efficientdet/efficientdet_d3-b0ea2cbc.pth')
    target_size = 2
...
class CustomEfficientDet(nn.Module):
    ...
    def forward(self, images, boxes=None, labels=None):
        if self.training:
            ...
            return self.model(images, torch.tensor([1] * images.shape[0]).float().cuda())
