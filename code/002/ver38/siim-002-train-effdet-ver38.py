...
def make_predictions(images, models, device, score_threshold):
    predictions = []
    images_list = [images, images.flip(-1), images.flip(-2), images.flip(-1, -2),
              images.transpose(-1, -2), images.transpose(-1, -2).flip(-1),
              images.transpose(-1, -2).flip(-2), images.transpose(-1, -2).flip(-1, -2)]
    with torch.no_grad():
        for i, model in enumerate(models):
            predictions_eachmodel = []
            outputs = model(images_list[i], image_scales=torch.tensor([1] * images_list[i].shape[0]).float().cuda())
            for i in range(images_list[i].shape[0]):
                # 画像一枚ごとにboxなどを出す
...
def inference(valid_loader, models, device, score_threshold):
    predictions = []
    for step, (image_ids, images, targets) in enumerate(valid_loader):
        images = torch.stack(images).float().to(device)
        predictions_temp = make_predictions(images, models, device, score_threshold)
        for i in range(images.shape[0]):
            boxes, scores, labels = run_wbf(predictions_temp, image_index=i, iou_thr=0.6, skip_box_thr=0)
...
def train_loop(folds, fold):
    ...
    for tta in range(CFG.tta):
        ...
        test_loader = DataLoader(test_dataset,
                                **CFG.loader_params['valid'],
                                collate_fn=collate_fn)
        predictions = inference(test_loader, models, device, score_threshold=1e-7)
