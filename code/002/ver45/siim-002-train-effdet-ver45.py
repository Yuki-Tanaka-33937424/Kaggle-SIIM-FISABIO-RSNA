...
class CFG:
    ...
    model_backbone_weight = os.path.join(ROOT_DIR, 'input/siim-010-data')
    target_size = 1
...
def get_model_train(fold):
    ...
    checkpoint = torch.load(os.path.join(CFG.model_backbone_weight,
                                         f'{CFG.backbone_name}_fold{fold}_best.pth'))['model']
    for key in list(checkpoint.keys()):
        if 'model.' in key:
            checkpoint[key.replace('model.', '')] = checkpoint[key]
            del checkpoint[key]
        if key.replace('model.', '') in remove_list:
            del checkpoint[key.replace('model.', '')]
...
def inference(valid_loader, models, device, iou_thr):
    predictions = []
    tta_transforms = []
    ...
    for step, (image_ids, images, targets) in tk0:
        ...
        for i in range(images.shape[0]):
            boxes, scores, labels = run_wbf(predictions_temp, image_index=i, iou_thr=iou_thr, skip_box_thr=0.01)
...
def train_loop(folds, fold):
    ...
    # model #
    # ====================================================
    model = get_model_train(fold=fold)
    model.to(device)
    ...
    test_loader = DataLoader(test_dataset,
                                **CFG.loader_params['valid'],
                                collate_fn=collate_fn)
    predictions = inference(test_loader, models, device, iou_thr=0.65)
...
def main():
    ...
    if CFG.train:
        ...
        annotations.to_csv(os.path.join(OUTPUT_DIR, 'annotations.csv'))
