get_ipython().system('pip install ../input/nfl-lib/timm-0.1.26-py3-none-any.whl')
get_ipython().system('tar xfz ../input/nfl-lib/pkgs.tgz')
# for pytorch1.6
...
class CFG:
    ...
    model_pretrained_weight = os.path.join(
        ROOT_DIR, 'input/efficientdet/efficientdet_d3-b0ea2cbc.pth')
    target_size = 1
...
def get_none_bboxes(predictions):

    predictions_ = predictions.copy()
    cols = ['conf', 'xmin', 'ymin', 'xmax', 'ymax']
    for col in cols:
        predictions_[col] = predictions_[col].apply(lambda x: float(x))

    oof_df_003 = pd.read_csv(os.path.join(ROOT_DIR, 'input/siim-003-data/oof_df.csv'))
    oof_df_003['preds_prob'] = oof_df_003['preds'].apply(lambda x: 1 / (1 + np.exp(-x)))
    oof_df_003['preds_label'] = oof_df_003['preds_prob'].apply(lambda x: 1 if x > 0.5 else 0)

    for i, id in enumerate(oof_df_003['id'].values.tolist()):
        if oof_df_003.loc[i, 'preds_label'] == 0:
            predictions_ = predictions_[predictions_['id'] != id]
            df = pd.DataFrame([{
                'id': id,
                'detection_label': '0',
                'conf': 1 - oof_df_003.loc[i, 'preds_prob'],
                'xmin': 0,
                'ymin': 0,
                'xmax': 1,
                'ymax': 1
            }])
            predictions_ = pd.concat([predictions_, df], axis=0)
    return predictions_
...
def train_loop(folds, fold):
    ...
    train_folds = train_folds[train_folds[CFG.target_col] == 1].reset_index(drop=True)
    valid_folds = valid_folds[valid_folds[CFG.target_col] == 1].reset_index(drop=True)
    ...
    predictions = make_predictions(valid_loader, model, device, score_threshold=0.01)
    annotations = valid_folds[['id', 'detection_label', 'xmin', 'ymin', 'xmax', 'ymax']].values
    ...
    if CFG.train:
        ...
        for fold in range(CFG.n_fold):
            if fold in CFG.trn_fold:
                ...
                losses.append(loss)
                annotations = pd.DataFrame(annotations, columns=['id', 'detection_label', 'xmin', 'ymin', 'xmax', 'ymax'])
                predictions = pd.DataFrame(predictions, columns=['id', 'detection_label', 'conf', 'xmin', 'ymin', 'xmax', 'ymax'])
                predictions = get_none_bboxes(predictions)
        ...
        LOGGER.info(f'mean of mAP {np.mean(mAPs):.4f}')
