...
class CFG:
    ...
    student_dir = os.path.join(ROOT_DIR, 'input/siim-013-data-local')
    pretrained = True
    target_size = 4
...
def train_fn(train_loader, model, criterion, optimizer, epoch, scheduler, device):
    ...
    for step, (images, labels) in enumerate(train_loader):
        ...
        with autocast(enabled=CFG.use_amp):
            if mix_decision < CFG.mixup_alpha:
        ...
        if (step + 1) % CFG.gradient_accumulation_steps == 0:
            if CFG.optimizer_name != 'SAM':
                ...
                if mix_decision < CFG.mixup_alpha:
                    loss_ = criterion(y_preds_, labels[0]) * labels[2] + criterion(y_preds_, labels[1]) * (1. - labels[2])
                    loss_.backward()
                else:
