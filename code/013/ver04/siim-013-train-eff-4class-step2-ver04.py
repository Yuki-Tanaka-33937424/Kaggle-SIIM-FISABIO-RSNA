...
class CFG:
    ...
    teacher = os.path.join(ROOT_DIR, 'output/012/ver04')
    pretrained = True
    target_size = 4
...
class SiimDataset(Dataset):
    def __init__(self, df=None, df_annotation=None, transform=None):
        self.df = df
    ...
    def __getitem__(self, idx):
        ...
        records = self.df_annotation[self.df_annotation['StudyInstanceUID'] == self.df.loc[idx, 'study_id']][['xmin', 'ymin', 'xmax', 'ymax']]
        for i in range(len(records)):
            ...
            image_annot = np.where(mask > 0, image_annot * 0.7 + mask * 0.3, image_annot).astype(np.float32)
        image /= 255.0
        image_annot /= 255.0
        if self.transform:
    ...
    for step, (images, images_annot, labels) in enumerate(train_loader):
        ...
        if (step + 1) % CFG.gradient_accumulation_steps == 0:
            if CFG.optimizer_name != 'SAM':
                ...
                features, _, y_preds_ = model(images)
                criterion(teacher_features, features, y_preds_, labels).backward()
...
def train_loop(folds, fold):
    ...
    train_dataset = SiimDataset(train_folds,
                                df_annotation=train_annotation,
    ...
    state = torch.load(os.path.join(CFG.teacher, f'{CFG.model_name}_fold{fold}_best.pth'))
