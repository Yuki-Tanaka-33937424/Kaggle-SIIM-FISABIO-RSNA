...
def train_loop(folds, fold):
    ...
    for epoch in range(CFG.epochs):
        ...
        annotations = get_annotations(valid_folds, CFG.target_col)
    ...
    if CFG.train:
        ...
        for fold in range(CFG.n_fold):
            if fold in CFG.trn_fold:
                ...
                annotations = get_annotations(_oof_df, CFG.target_col)
                predictions = get_predictions(_oof_df, col='preds')
                mAP, AP = mean_average_precision_for_boxes(
                    annotations, predictions, iou_threshold=0.5, exclude_not_in_annotations=False, verbose=False)
                LOGGER.info(f'Class: {CFG.target_col} AP: {mAP:.4f}')
