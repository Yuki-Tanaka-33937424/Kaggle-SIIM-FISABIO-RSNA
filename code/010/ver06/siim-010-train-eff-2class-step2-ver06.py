...
class CFG:
    ...
    teacher_dir = os.path.join(ROOT_DIR, 'output/009/ver05')
    pretrained = True
    target_size = 1
    ...
    for step, (images, images_annot, labels) in enumerate(train_loader):
        ...
        if (step + 1) % CFG.gradient_accumulation_steps == 0:
            if CFG.optimizer_name != 'SAM':
                ...
                features_, _, y_preds_ = model(images)
                criterion(teacher_features, features_, y_preds_, labels).backward()
...
def train_loop(folds, fold):
    ...
    state = torch.load(os.path.join(CFG.teacher_dir, f'{CFG.model_name}_fold{fold}_best.pth'))
