...
class CFG:
    ...
    teacher = os.path.join(ROOT_DIR, 'input/siim-009-data/tf_efficientnet_b5_ns_fold0_best.pth')
    pretrained = True
    target_size = 1
...
def get_train_file_path(image_id):
    return os.path.join(ROOT_DIR, f"input/siim-covid19-resized-to-512px-png/train/{image_id}.png")


def get_test_file_path(image_id):
    return os.path.join(ROOT_DIR, f"input/siim-covid19-resized-to-512px-png/test/{image_id}.png")
...
class SiimDataset(Dataset):
    ...
    def __getitem__(self, idx):
        ...
        records = self.df[self.df['id'] == self.df.loc[idx, 'id']][['xmin', 'ymin', 'xmax', 'ymax']]
        for i in range(len(records)):
            record = records.iloc[i].astype(int)
            mask = np.full((512, 512, 3), 0, dtype=np.uint8)
            cv2.rectangle(mask, (record['xmin'], record['ymin']), (record['xmax'], record['ymax']), (255, 0, 0), thickness=-1)
            image_annot = np.where(mask > 0, image_annot * 0.7 + mask * 0.3, image).astype(np.float32)
        ...
        return image.float(), image_annot.float(), label.float()
...
class CustomEfficientNet(nn.Module):
    ...
    def forward(self, x):
        ...
        return features, all_features, torch.squeeze(output)
...
class CustomLoss(nn.Module):
    ...
    def forward(self, teacher_features, features, y_pred, labels):
        ...
        cls_loss = get_criterion()(y_pred, labels)
...
def train_fn(train_loader, teacher_model, model, criterion, optimizer, epoch, scheduler, device):
    ...
    for step, (images, images_annot, labels) in enumerate(train_loader):
        ...
        images_annot = images_annot.to(device)
        labels = labels.to(device)
        batch_size = labels.size(0)
        with torch.no_grad():
            teacher_features, _, _ = teacher_model(images_annot)
...
def train_loop(folds, fold):
    ...
    model.to(device)
    teacher_model = CustomEfficientNet(CFG.model_name, pretrained=False)
